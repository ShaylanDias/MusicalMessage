...
from tools import htmlFindAll, filtr, ParseLyric, ParseSongs, ParseSearch
...
class AZlyrics(Requester):
    ...
    def getLyrics(self, title=None, artist=None, search=False, url=None, ext='txt', save=True, sleep=5, dir='./'):
        ...
        try:
            html = htmlFindAll(page)
            x = html('div', class_='lyricsh')
            y = x[0]
            string = y.getText().strip()[:-7]
            artist = filtr(string, True)
            # Gets the lyrics div, then grabs the first section which is artist in form "\nArtistName Lyrics\n" and strips the \n and Lyrics parts.
            artist = filtr(html('div', class_='lyricsh')[0].getText().strip()[:-7], True)
        ...
        if lyrics:
            if save:
                print(dir+'{} - {}.{}'.format(title.title(), artist.title(), ext))
                with open(dir+'{} - {}.{}'.format(title.title(), artist.title(), ext), 'w', encoding='utf-8') as f:
